...
describe('useHybridScripts ssr specs', () => {
  ...
  it('it should add css links to to ssr context', () => {
    ...
    const renderedSrrContext = ssrContext.hybridScripts.render();
    expect(renderedSrrContext).toContain(`<script>function onHybridScriptLoaded(el) { el.setAttribute('data-hybrid-script-loaded', 'true');window.dispatchEvent(new CustomEvent('hybrid-script-loaded'));}</script>`);
    expect(renderedSrrContext).toContain(`<link href=\"https://code.jquery.com/ui/1.13.0/themes/base/jquery-ui.css\" data-hybrid-script-id=\"https://code.jquery.com/ui/1.13.0/themes/base/jquery-ui.css\" rel=\"stylesheet\" onload=\"onHybridScriptLoaded(this)\">`);
  });
  ...
  it('it should support script and link objects as arguments', () => {
    ...
    const renderedSrrContext = ssrContext.hybridScripts.render();
    expect(renderedSrrContext).toContain(`<link href=\"http://localhost:8080/static/test.css\" data-hybrid-script-id=\"http://localhost:8080/static/test.css\" rel=\"stylesheet\" onload=\"onHybridScriptLoaded(this)\">`);
